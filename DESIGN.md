...
In dark mode the canvas inverts: deep green-black backgrounds replace white, and Near Black becomes the accent text color on bright surfaces. Wise Green stays identical — it reads just as well on dark as on light.

**Backgrounds**
- **Surface Base** (`#0e0f0c`): Page background — the same Near Black used for light-mode text
- **Surface Raised** (`#161a13`): Cards, modals, elevated panels — a warmer, slightly lighter dark green
- **Surface Overlay** (`#1e2419`): Dropdowns, popovers, highest elevation surfaces

**Text**
- **Primary Text** (`#f0f4ed`): Headlines and body — warm off-white with a faint green cast
- **Secondary Text** (`#a8afa4`): Captions, labels, muted content
- **Tertiary Text** (`#666b63`): Placeholder, disabled states

**Brand Accent** (unchanged)
- **Wise Green** (`#9fe870`): Primary CTA — identical in dark mode, no adjustment needed
- **Dark Green** (`#163300`): Button text on green — unchanged

**Semantic (dark-adjusted)**
- **Positive Green** (`#4caf7d`): Success text/icons on dark — lightened from `#054d28` for contrast
- **Danger Red** (`#f2686d`): Error/destructive on dark — lightened from `#d03238`
- **Warning Yellow** (`#ffd11a`): Warnings — unchanged, sufficient contrast on dark
- **Info Tint** (`rgba(56,200,255,0.15)`): Slightly more opaque than light-mode tint

**Borders & Surfaces**
- **Border Subtle** (`rgba(240,244,237,0.10)`): Card and container borders
- **Border Emphasis** (`rgba(159,232,112,0.30)`): Green accent borders (highlighted cards)
- **Green Tint Surface** (`rgba(159,232,112,0.08)`): Badge backgrounds, hover fills

**Hover & Interaction**
- **Nav Hover** (`rgba(159,232,112,0.12)`): Navigation item hover — green tint on dark
- **Button Hover Scale**: `scale(1.05)` — unchanged, animation is the same
